  ...
  export class OfferDataService {
    ...
    public getOffers(): Observable<OfferPreview[]> {
      return this.http.get<OfferPreview[]>(`${BASE_URL}/${APIRoute.OFFERS}`).pipe(...defaultHttpOperators<OfferPreview[]>());
    }

    public getOfferById(offerId: string): Observable<Offer> {
      return this.http.get<Offer>(`${BASE_URL}/${APIRoute.OFFERS}/${offerId}`).pipe(...defaultHttpOperators<Offer>());
    }

    public getNearbyOffers(offerId: string): Observable<OfferPreview[]> {
      return this.http.get<OfferPreview[]>(`${BASE_URL}/${APIRoute.OFFERS}/${offerId}/${API_PATHS.NEARBY}`).pipe(...defaultHttpOperators<OfferPreview[]>());
    }
  }
